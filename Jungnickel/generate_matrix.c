    ...
    while (edges < n - 1)
    {
        ...
        if (matrix[min_v][max_v] == 0)
        {
            ...
            printf("Edge %d: Connected %d and %d (degrees: %d, %d)\n", 
                  edges, min_v + 1, max_v + 1, temp[min_v] + 1, temp[max_v] + 1);
        }
        else
        {
            ...
            if (temp[min_v] > 0) temp[min_v]--;
            else if (temp[max_v] > 0) temp[max_v]--;
            else {
                // 両方0なら探索終了
                printf("Error: Cannot create more edges. Graph may not be constructible with this algorithm.\n");
                break;
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            if (temp[i] > 0) remaining_vertices++;
        }
        
        if (remaining_vertices < 2 && edges < n - 1) {
            printf("Warning: Not enough vertices with positive degree remaining, but only %d edges created.\n", edges);
            printf("Original degrees: ");
            for (int i = 0; i < n; i++) printf("%d ", orig_degs[i]);
            printf("\nCurrent degrees: ");
